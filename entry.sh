...
if bashio::services.available "mqtt"; then
    bashio::log.info "MQTT service discovered! Using auto-discovered settings..."
    ...
    bashio::log.green "Auto-discovered MQTT broker at ${MQTT_HOST}:${MQTT_PORT}"
else
    bashio::log.warning "MQTT service not discovered, using manual configuration..."
    ...
    # Check if MQTT host is configured
    if [ -z "$MQTT_HOST" ]; then
        bashio::log.fatal "MQTT host not configured and auto-discovery failed!"
        bashio::log.fatal "Please configure MQTT settings or enable MQTT integration in Home Assistant"
        exit 1
    fi
fi
