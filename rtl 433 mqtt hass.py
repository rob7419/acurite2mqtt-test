...
# Track if we've published the gateway device
...
def publish_gateway_device(mqttc):
    ...
    global gateway_published
    
    if gateway_published:
        return
    
    # Create a sensor to track gateway status
    device_type = "sensor"
    object_id = "rtl433_gateway"
    object_suffix = "status"
    
    path = "/".join([DISCOVERY_PREFIX, device_type, object_id, object_suffix, "config"])
    
    config = {
        "name": "RTL-433 Gateway Status",
        "unique_id": "rtl433_gateway_status",
        "state_topic": "/".join([MQTT_TOPIC, "status"]),
        "availability_topic": "/".join([MQTT_TOPIC, "status"]),
    ...
    # Define the gateway device
    device = {
        "identifiers": ["rtl433_gateway"],
    ...
    logging.info("Published RTL-433 Gateway device for sensor grouping")
...
def publish_config(mqttc, topic, model, instance, channel, mapping):
    ...
    # Create sensor device (the individual sensor)
    sensor_device = {
        "identifiers": [instance],
        "name": f"{model.replace('-', ' ')} {instance}",
        "model": model_name,
        "manufacturer": manufacturer,
        "via_device": "rtl433_gateway"  # Link to gateway device
    }
    
    config["device"] = sensor_device
